...
function commit() {
    docker exec -e CORE_PEER_MSPCONFIGPATH=$CORE_PEER_MSPCONFIGPATH cli peer lifecycle chaincode commit -o orderer.example.com:7050 --channelID my-channel --name test-contract-2 --version 1.0.0 --sequence 1 --peerAddresses peer0.org1.example.com:7051
}
